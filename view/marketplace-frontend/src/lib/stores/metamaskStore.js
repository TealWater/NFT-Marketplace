...
//Metamask store
const getMetaMaskPresent = () => typeof window !== "undefined" && typeof window.ethereum !== "undefined";
...
export function MetaMaskStore() {
    ...
    const hanldeAccountsChanged = (/**@type {any} */ newAccounts) => {
        if (
   ...
   */
    const init = async () => {
        // if metamask !present, do nothing
        if (!getMetaMaskPresent()) {
            return;
        }

        // if we have an account in loacal storage, we can assume the user left the page with metamask connected last time so we can  connect to metamask automatically
        if (getLocalStorageState().account) {
            ...
            if (accountResponse && accountResponse.length && accountResponse[0]) {
                ...
                //watch for account changes for the extension
                window.ethereum?.on("accountsChanged", hanldeAccountsChanged);
            }
        }

        // if metamask is present, but not connected previously, we will wait for the to click connect. For now, we will just set loaded to true.
        setTimeout(() => {
    ...
    const connect = async () => {
        ...
        //if MetaMask is present but !connected, connect
        const accountResponse = await window.ethereum?.request({
            method: "eth_requestAccounts",
        });
